...
export function findLegacyLeague(input) {
...
export function matchesLeagueSelection(team, leagueIds = []) {
  ...
  const legacyLeague = findLegacyLeague(team);
  return legacyLeague ? normalizedLeagueIds.includes(String(legacyLeague.id)) : false;
}

export function buildLeagueCatalog(teams = [], leagueIds) {
  const filtered = leagueIds?.length
    ? teams.filter((team) => matchesLeagueSelection(team, leagueIds))
    : teams;
